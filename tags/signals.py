...
def update_tag_correlations_from_tag_list(tag_list):
    """
    Recebe uma lista de tags e atualiza correlações entre pares únicos (sem duplicidade bidirecional).
    """
    tags = list(UUIDTag.objects.filter(id__in=[t.id if isinstance(t, UUIDTag) else t for t in tag_list]))

    for i in range(len(tags)):
        for j in range(i + 1, len(tags)):  # evita repetições e auto-relacionamentos
            tag1, tag2 = sorted([tags[i], tags[j]], key=lambda t: str(t.id))  # ordena por ID para manter consistência
